...
app.post("/api/webhook/dodopayment", express.raw({ type: "application/json" }), async (req, res) => {
  console.log("HEADERS >>>", req.headers);
  console.log("CONTENT-TYPE >>>", req.headers["content-type"]);
  console.log("RAW BODY (buffer len) >>>", req.body?.length);
  try {
    ...
    async function upsertSubscription(subData: any) {
      ...
      const plan =
        subData.payment_frequency_interval === "Month"
          ? "monthly"
          : subData.subscription_period_interval?.toLowerCase().includes("year")
          ? "yearly"
    ...
    switch (type) {
      ...
      case "refund.succeeded": {
        ...
        const refundPaymentId =
          data.payment_id || data.paymentId || data.payment?.payment_id || null;
