...
io.on("connection", (socket) => {
  ...
  /**
   * Generates and streams audio for a given text.
  ...
  /**
   * The core logic handler for processing a user's final transcript.
  ...
  const handleUserResponse = async (transcript: string) => {
    ...
    try {
      ...
      console.log("🎙️ Generating AI decision...");
      ...
      // --- YOUR APPLICATION LOGIC EXECUTES THE AI'S DECISION ---
  ...
  socket.on("start-stream", () => {
    ...
    deepgramLive.on(LiveTranscriptionEvents.Transcript, (data: any) => {
      ...
      if (transcript && !data.is_final) {
        isUserSpeaking = true;
        console.log("🗣️ User said (interim):", transcript);
